...
public class 架构数据 { public string? 路径, 选架构, 选机器, 选处理器, 选加速 = "自动", 翻译块大小, 逻辑核心, 插槽数, 核心数, 线程数; public List<string> 架构列表 = [], 机器列表 = [], 处理器列表 = [], 加速列表 = ["自动", "TCG", "TCG(多线程)", "WHPX", "WHPX+TCG"]; public bool 加载中; }
public partial class 架构 : Grid
{
    ...
    public 架构()
    {
        ...
        _逻辑框.TextChanged += (s, e) => 数据.逻辑核心 = _逻辑框.Text; _插槽框.TextChanged += (s, e) => 数据.插槽数 = _插槽框.Text;
        _核心框.TextChanged += (s, e) => 数据.核心数 = _核心框.Text; _线程框.TextChanged += (s, e) => 数据.线程数 = _线程框.Text;
        _翻译块框.TextChanged += (s, e) => 数据.翻译块大小 = _翻译块框.Text; Children.Add(容器);
    }
    public string 拼命令(仿真配置 配置)
    {
        ...
        if (数据.选加速 is string 模式 && 模式 != "自动")
        {
            ...
            else if (模式.StartsWith("TCG")) { var 参数 = 模式.Contains("多线程") ? ",thread=multi" : ""; if (!string.IsNullOrWhiteSpace(数据.翻译块大小)) 参数 += $",tb-size={数据.翻译块大小}"; 命令行 += $" -accel tcg{参数}"; }
        }
        ...
        if (内存.节点 > 1) for (int i = 0; i < 内存.节点; i++) 命令行 += $" -numa node,mem={内存.容量 / 内存.节点}M,cpus={i},nodeid={i}"; else 命令行 += " -machine memory-backend=ram0";
        if (!string.IsNullOrEmpty(磁盘.路径)) 命令行 += $" -drive file=\"{磁盘.路径}\",if={磁盘.接口.ToLower()},index=0{(磁盘.模式 != 2 ? $",format={磁盘.格式}" : "")}";
        var 光盘列表 = 光盘.实例.配置.路径列表;
        for (int i = 0; i < 光盘列表.Count; i++) if (!string.IsNullOrEmpty(光盘列表[i])) 命令行 += $" -drive file=\"{光盘列表[i]}\",media=cdrom,index={i + 1}";
        ...
        if (内存.气球) 命令行 += " -device virtio-balloon"; if (内存.加密) 命令行 += " -object sev-guest,id=sev0 -machine memory-encryption=sev0";
        return 命令行;
    }
    public async Task 更新架构(string 架构名)
    {
        ...
        {
            数据.加载中 = true; 刷新UI(); var 程序 = Path.Combine(数据.路径!, $"qemu-system-{架构名}.exe");
            var 结果 = await Task.WhenAll(运行(程序, "-machine help"), 运行(程序, "-cpu help"));
            _缓存[架构名] = 列表 = (解析(结果[0]), 解析(结果[1])); 数据.加载中 = false;
        }
        (数据.机器列表, 数据.处理器列表) = 列表; (数据.选机器, 数据.选处理器) = (列表.Item1.FirstOrDefault(), 列表.Item2.FirstOrDefault());
        刷新UI(); _ = 网络.实例.刷新设备(); _ = 显示.实例.刷新设备();
    }
    private static async Task<string> 运行(string 程序, string 参数) { try { using var 进程 = Process.Start(new ProcessStartInfo(程序, 参数) { RedirectStandardOutput = true, CreateNoWindow = true }); return 进程 is null ? "" : await 进程.StandardOutput.ReadToEndAsync(); } catch { return ""; } }
    public async Task 扫描路径(string 根路径) { 数据.路径 = 根路径; 数据.加载中 = true; 刷新UI(); if (new DirectoryInfo(根路径) is { Exists: true } 目录) { 数据.架构列表 = [.. 目录.GetFiles("qemu-system-*.exe").Where(f => !f.Name.EndsWith("w.exe")).Select(f => f.Name[12..^4]).OrderBy(n => n)]; if (数据.架构列表.Count > 0) await 更新架构(数据.架构列表[0]); } 数据.加载中 = false; 刷新UI(); }
    public void 刷新UI() { _进度条.Visibility = (Visibility)(数据.加载中 ? 0 : 1); 绑定(_架构框, 数据.架构列表, 数据.选架构); 绑定(_机器框, 数据.机器列表, 数据.选机器); 绑定(_处理器框, 数据.处理器列表, 数据.选处理器); }
    ...
    private static void 绑定(ComboBox 下拉框, List<string> 数据源, string? 选中值) { if (下拉框.ItemsSource != 数据源) 下拉框.ItemsSource = 数据源; 下拉框.SelectedItem = (选中值 != null && 数据源.Contains(选中值)) ? 选中值 : (数据源.Count > 0 ? 数据源[0] : null); }
}