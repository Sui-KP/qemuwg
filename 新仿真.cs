...
public partial class 新仿真 : Grid
{
    ...
    private void 切换(int i)
    {
        ...
        _标题.Text = i switch { 0 => "架构设置", 1 => "内存配置", 2 => "存储磁盘", _ => "命令预览" };
        if (i == 3)
        {
            var p = new 仿真配置 { 内存 = _内存视图.配置, 磁盘 = _磁盘视图.配置 };
        ...
        {
            UIElement view = i switch { 0 => _架构视图, 1 => _内存视图, 2 => _磁盘视图, _ => new Grid() };
